...
class TestRunnerPlugin(Plugin):
    ...
    def __init__(self, application=None):
        ...
        self._stress_event = None
        self._stress_checkbox = False
    ...
    def _register_actions(self):
        run_action_info = ActionInfo("Tools", "Run Tests", self.OnRun, None,
                                     "F8", ImageProvider().TOOLBAR_PLAY, "Run the selected tests", position=11)
        self._run_action = self.register_action(run_action_info)
        stop_action_info = ActionInfo("Tools", "Stop Test Run", self.OnStop, None,
                                      "CtrlCmd-F8", ImageProvider().TOOLBAR_STOP, "Stop a running test",position=12)
    ...
    def OnStressTestCheckbox(self, evt):
        '''Called when the user clicks on the "Stress Test" checkbox'''
        self.save_setting("stress_test", evt.IsChecked())
    ...
    def OnTimer(self, evt):
        ...
        if not self._test_runner.is_running():
            ...
            if self.stress_test:
                if not self._progress_bar.get_fail():
                    wx.CallLater(2000, self.OnRun, self._stress_event)
            return
    ...
    def _build_local_toolbar(self):
        ...
        # but effective.
        self.stress_test_cb = wx.CheckBox(toolbar, ID_STRESSTEST, " StressTest  ")
        self.stress_test_cb.SetToolTip(wx.ToolTip("Automatically run all testcases forever"))
        self.stress_test_cb.SetValue(self.stress_test)
        toolbar.AddControl(self.stress_test_cb)
    ...
    def _bind_toolbar_events(self, toolbar):
        for event, callback, id in (
            ...
            toolbar.Bind(event, callback, id=id)
        toolbar.Bind(wx.EVT_CHECKBOX, self.OnStressTestCheckbox, self.stress_test_cb)
